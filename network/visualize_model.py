...
writer_1 = SummaryWriter('runs/t-SNE-1')
writer_2 = SummaryWriter('runs/t-SNE-2')
best_prec1 = 0
...
def validate(val_loader, model, criterion):
    ...
    for i, (input, target) in enumerate(val_loader):
        ...
        class_labels.append(classes[target])
        fc_etxractor = FeatureExtractor(model, extract_list)
        with torch.no_grad():
            extracted_results = fc_etxractor(input)
        # features.append(features, extracted_results["avgpool"],dim=1)
        features = torch.cat((features, extracted_results["avgpool"]),dim=0)
